...
describe('concat',()=>{
  ...
  it('Should return concatenation of two strings', () => {
      expect(concat('abc','def')).to.equal('abcdef');
  });

  it( 'should throw error if first parameter is empty', () => {
    expect( () => {concat('','def')}).to.throw('Parameters can not be empty.');
  });

  it( 'should throw error if second parameter is empty', () => {
    expect( () => {concat('abc','')}).to.throw('Parameters can not be empty.');
  });
});